...
export default function WalletCard({ userRole }) {
  ...
  const handleWithdrawReq = async (e) => {
    ...
    if (!nameConfirmed) {
      toast.error(
        ...
        "Please confirm the account name before submitting.",
      );
      return;
    }
    try {
  ...
  if (error)
    return (
      <div className="card text-center" style={{ padding: "var(--space-8)" }}>
        <AlertTriangle
          style={{ color: "var(--color-error)", margin: "0 auto" }}
        />
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: "var(--text-xs)", color: "#78350f" }}>
              You must complete identity verification before requesting a
              withdrawal.
            </p>
            <Link
              href={`/${String(userRole || "").toLowerCase()}/profile`}
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 4,
            ...
            {resolvedName && !nameConfirmed && (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  >
                    <CheckCircle size={13} style={{ marginRight: 4 }} /> Yes,
                    confirm
                  </button>
                  <button
                    type="button"
