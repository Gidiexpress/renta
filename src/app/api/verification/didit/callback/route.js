  ...
  try {
    ...
    if (!sessionId) {
      ...
      return NextResponse.redirect(
        `${appUrl}/tenant/profile?verification=error`,
      );
    }
    ...
    const user = await prisma.user.findFirst({
      ...
      select: { id: true, ninStatus: true, role: true },
    });
    ...
    const rolePath = String(user.role || "TENANT").toLowerCase();
    ...
    if (!apiKey) {
      ...
      return NextResponse.redirect(
        `${appUrl}/${rolePath}/profile?verification=pending`,
      );
    }

    const decisionRes = await fetch(
      `${DIDIT_BASE}/v3/session/${sessionId}/decision/`,
      {
    ...
    console.log(
      `[Didit Callback] Decision response (${decisionRes.status}):`,
      decisionText,
    );
    ...
    if (!decisionRes.ok) {
      ...
      console.warn(
        "[Didit Callback] Decision not ready for session:",
        sessionId,
    ...
    return NextResponse.redirect(
      `${appUrl}/${rolePath}/profile?verification=${outcome}`,
    );
  } catch (error) {
