...
import { Star, Calendar, ExternalLink, ShieldCheck, AlertCircle, Search, MoreVertical, CheckCircle2 } from 'lucide-react';
    ...
    return (
        <div className="fade-in">
            <div className={styles.welcomeSection}>
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-muted">Manage featured listings and boost property visibility</p>
                    </div>
                    <div className="bg-primary-light p-4 rounded-2xl flex items-center gap-4">
                        <div className="text-right">
                            <span className="block text-xs uppercase text-muted font-bold tracking-wider">Active Featured</span>
                            <span className="text-2xl font-black text-primary">{featuredCount}</span>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    ...
                    <div className="col-span-full py-20 text-center">
                        ...
                        <p className="text-muted">Search for verified properties to promote them.</p>
                    </div>
                ) : (
                    filteredProperties.map((property) => (
                        <div key={property.id} className={`card overflow-hidden transition-all duration-300 ${property.isFeatured ? 'border-primary ring-1 ring-primary-light ring-offset-2' : ''}`}>
                            <div className="relative aspect-video">
                                {property.images?.[0] ? (
                                    <img src={property.images[0].url} alt={property.title} className="w-full h-full object-cover" />
                                ) : (
                                    <div className="w-full h-full bg-gray-100 flex items-center justify-center text-muted">No Image</div>
                                )}
                                {property.isFeatured && (
                            ...
                            <div className="p-5">
                                <div className="flex justify-between items-start mb-2">
                                    <h3 className="font-bold line-clamp-1 flex-1 pr-2">{property.title}</h3>
                                    <Link href={`/listing/${property.id}`} target="_blank" className="text-muted hover:text-primary">
                                ...
                                {property.isFeatured && property.featuredUntil && (
                                    <div className="bg-success-light text-success text-xs p-2 rounded-lg mb-4 flex items-center gap-2">
                                        <Calendar size={14} />
                                        <span>Expires: {new Date(property.featuredUntil).toLocaleDateString()}</span>
                                    </div>
                                )}
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all ${property.isFeatured
            ...
            <style jsx>{`
                ...
                .bg-success-light { background-color: #f0fdf4; border: 1px solid #dcfce7; }
                .line-clamp-1 { display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden; }
                .ring-primary-light { --tw-ring-color: rgba(var(--color-primary-rgb), 0.2); }
            `}</style>
        </div>
    );
}
